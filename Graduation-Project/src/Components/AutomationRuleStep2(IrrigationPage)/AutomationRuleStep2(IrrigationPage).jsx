...
const AutomationRuleStep2IrrigationPage = (children) => {
    ...
    const validationSchema=object({
            ...
            async function sendAutomationRule(values){
                try {
                    const options={
                        url:`https://agrivision.tryasp.net/farms/${children.farm.farmId}/fields/${children.field.id}/AutomationRules`,
                        method:"POST",
                        data:values,
                        headers:{
    ...
    return (
        <section className='h-[100vh]  flex justify-center items-center bg-black bg-opacity-70  font-manrope  absolute z-50 w-[100%]' onClick={(e)=>{if(e.target==e.currentTarget){setControlIrrigationPage(null)}}}>
            <div className="w-[650px] px-[40px] min-h-[500px]   border-2 rounded-2xl bg-white p-[20px]">
                ...
                <form action="" className="w-[100%] mt-[30px] flex flex-col justify-between  pe-[10px] text-[18px] font-s flex-grow" onSubmit={formik.handleSubmit}>
                        <div className="flex flex-col gap-3 my-5">
                            ...
                            <div className="">
                                ...
                                <MenuElement Items={type} nameChange={type[typeId]} setIndex={setTypeId} onList={onList} width={100+"%"} name={"Check Fields"} setOnList={setOnList} Pformat={"text-[#616161]"}/>
                            </div>
                            {typeId==1?<div className="">
                                ...
                                <div className="">
                                    ...
                                    <div className="my-[10px] flex justify-between px-[15px] text-[18px] font-semibold">
                                        <div className="w-[40px] h-[40px] rounded-full bg-[#c2c2c23f] flex justify-center items-center" onClick={()=>{setDays(prev=>({...prev,Mon:1}))} }>M</div>
                                        <div className="w-[40px] h-[40px] rounded-full bg-mainColor flex justify-center items-center text-white" onClick={()=>{setDays(prev=>({...prev,Tue:2}))} }>T</div>
                                        <div className="w-[40px] h-[40px] rounded-full bg-mainColor flex justify-center items-center text-white" onClick={()=>{setDays(prev=>({...prev,Wed:4}))} }>W</div>
                                        <div className="w-[40px] h-[40px] rounded-full bg-[#c2c2c23f] flex justify-center items-center" onClick={()=>{setDays(prev=>({...prev,Thu:8}))} }>T</div>
                                        <div className="w-[40px] h-[40px] rounded-full bg-[#c2c2c23f] flex justify-center items-center" onClick={()=>{setDays(prev=>({...prev,Fri:16}))} }>F</div>
                                        <div className="w-[40px] h-[40px] rounded-full bg-mainColor flex justify-center items-center text-white" onClick={()=>{setDays(prev=>({...prev,Sat:32}))} }>S</div>
                                        <div className="w-[40px] h-[40px] rounded-full bg-[#c2c2c23f] flex justify-center items-center" onClick={()=>{setDays(prev=>({...prev,Sun:64}))} }>S</div>
                                    </div>
                                </div>
                            </div>:<div className="">
                                <div className="">
