...
const Personal = () => {
  ...
  async function getAccountData() {
    if (!token) {
      console.error("No token available for getAccountData");
      return;
    }
    try {
      const options = {
  ...
  async function updateAccountData() {
    if (!token) {
      console.error("No token available for updateAccountData");
      return;
    }
    let values = {
  ...
  useEffect(() => {
    // Handle firstName state changes if needed
  }, [firstName]);
  return data ? (
        ...
        <div className="">
          <div className="border-t-[1px] border-[#0d121c3c] py-[15px]">
            <div className="flex justify-between items-center">
              <h3 className="mb-[12px] text-[17px] font-semibold">
                First Name
              </h3>
              <div className="">
                {!firstName.state ? (
                  <div
                    className="flex items-center space-x-2 hover:text-mainColor transition-all duration-150 cursor-pointer"
                    onClick={() => {
                      setFirstName((prev) => ({ ...prev, state: 1 }));
                    }}
                  >
                  ...
                  <div className="flex  items-center space-x-10">
                    ...
                    <div
                      className="flex items-center space-x-2 hover:text-red-500 transition-all duration-150 cursor-pointer"
                      onClick={() => {
                        setFirstName({ value: "", state: 0 });
                      }}
                    >
                      <X strokeWidth={1.8} />
            ...
            {firstName.state ? (
              <input
                type="text"
                className="w-full py-[8px] px-[15px] rounded-[10px] border-[2px] border-[#9F9F9F] placeholder:text-[15px]"
                placeholder={data.firstName}
                onChange={(e) => {
                  setFirstName((prev) => ({
                    ...prev,
                    value: e.target.value ? e.target.value : firstName.value,
                  }));
                }}
              />
            ) : (
            ...
            {lastName.state ? (
              <input
                ...
                onChange={(e) => {
                  setLastName((prev) => ({
                    ...prev,
                    value: e.target.value ? e.target.value : lastName.value,
                  }));
                }}
              />
            ) : (
            ...
            {username.state ? (
              <input
                ...
                onChange={(e) => {
                  setUsername((prev) => ({
                    ...prev,
                    value: e.target.value ? e.target.value : username.value,
                  }));
                }}
              />
            ) : (
            ...
            {email.state ? (
              <input
                ...
                onChange={(e) => {
                  setEmail((prev) => ({
                    ...prev,
                    value: e.target.value ? e.target.value : email.value,
                  }));
                }}
              />
            ) : (
            ...
            {phoneNumber.state ? (
              <input
                ...
                onChange={(e) => {
                  setPhoneNumber((prev) => ({
                    ...prev,
                    value: e.target.value ? e.target.value : data.phoneNumber,
                  }));
                }}
              />
            ) : (
