...
const EditIrrigation = (children) => {
        let {baseUrl}=useContext(AllContext);
        let {token}=useContext(userContext)
        let Name=useRef(null);
        let SerialNumber=useRef(null)
        let [irrigationList,setIrrigationList]=useState([])
    ...
    try {
        ...
        irrigationList.splice(0,1)
        irrigationList.push({Name:data.name,SerialNumber:data.serialNumber})
    ...
    useEffect(()=>{getIrrigationUnits()},[])
        function addIrrigationData(){
            const alreadyExists = irrigationList.some(
            (member) => member.Name === Name.current.value
            );
            if(!Name.current.value||!SerialNumber.current.value){
                toast.error("Irrigation unit data is incomplete");
                return;
            }
            if (alreadyExists) {
                toast.error("The irrigation unit is already there.");
                return;
            }
            irrigationList.push({
                ...
                if(e.target===e.currentTarget){
                    ...
                    <form action="" className="w-[85%] my-5 flex flex-col justify-between  text-[18px] flex-grow">
                        <div className="flex flex-col gap-3 my-5">
                            <div className="">
                                <label htmlFor="" className='ms-1'>Irrigation Unit Name</label>
                                <input type="text" placeholder='Enter Irrigation unit name ' ref={Name}  className='formControl mx-0 rounded-xl text-[16px] py-5 w-[100%] border-[#0d121c21] '/>
                            </div>
                            ...
                            <i className="fa-solid fa-plus me-2"></i>Add Irrigation Unit</button>
                        </div>
                        ...
                        {children.FieldData?.IrrigationUnit?.map((item,index)=>{
                            ...
                            <div className="bg-[#1e693021] rounded-lg py-3 px-5 my-1 flex justify-between items-center">
                                ...
                                <X className="fa-solid fa-x text-[#9F9F9F] hover:text-red-600 transition-colors duration-300 " onClick={()=>{
                                    setIrrigationList(irrigationList.filter((_, i) => i !== index));
                                    children.setFieldData(prev=>({
                                        ...prev,
                                        IrrigationUnit:children.FieldData.IrrigationUnit.filter((_, i) => i !== index)
                                    }))
                                }}/>
                            </div>
                        </div>})}
                        ...
                        <div className="flex justify-between items-center mt-[10px]">
                            ...
                            <button type='button'  className="btn2 self-end w-[150px] rounded-lg py-4 bg-mainColor text-[16px] text-white hover:bg-transparent hover:border-mainColor border-2 hover:text-mainColor font-medium " onClick={()=>{
