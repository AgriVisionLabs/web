...
const ShowTask = (children) => {
    ...
    let completedOrNot=[{color:"#0D121C",bg:"bg-[#fcfbfb00]"},{color:"#f5f5f7",bg:"bg-[#25C462]"}]
        ...
        {taskData?<section className='inset-0  flex justify-center items-center bg-black bg-opacity-70  font-manrope  absolute z-50 w-[100%]' onClick={(e)=>{if(e.target==e.currentTarget){children.setDisplayTask(null)}}}>
            <div className="w-[700px] px-[40px] h-[730px] overflow-hidden   border-2 rounded-2xl bg-white pt-[40px] pb-[10px] text-[#0D121C] font-manrope">
                ...
                <div className="bg-[#F4F4F4] rounded-[15px] my-[18px] p-[24px] h-[80px] overflow-y-auto">
                    <p className="text-[18px] capitalize  font-medium">{taskData.description}</p>
                </div>
                <div className=" grid grid-cols-2 space-y-[10px]">
                    <div className="space-y-[8px]">
                        ...
                        <div className="flex  items-center space-x-2 font-semibold">
                            <MapPin/>
                            <p className="text-[17px]  text-[#0D121C]">{children.farmData.name} - {taskData.fieldName}</p>
                        </div>
                    </div>
                    <div className="space-y-[8px]">
                        ...
                        <div className="flex  items-center space-x-2 font-semibold">
                            <Calendar/>
                            <p className="text-[17px] text-[#0D121C]"><DateDisplay dateStr={taskData.dueDate}/></p>
                        </div>
                    </div>
                    <div className="space-y-[8px]">
                        ...
                        <div className="flex  items-center space-x-2 font-semibold rounded-[15px] border-[2px] border-[#0d121c61] w-fit px-[12px] py-[4px]">
                            <p className={`text-[17px] text-[${completedOrNot[taskData.completedAt?1:0].color}]  ${completedOrNot[taskData.completedAt?1:0].bg}  capitalize`} >{taskData.completedAt?"completed":"not completed"}</p>
                        </div>
                    </div>
                    <div className="space-y-[8px]">
                        <p className="text-[17px] font-bold text-[#616161]">Priority</p>
                        <div className={`flex  items-center space-x-2 font-semibold rounded-[15px] bg-[${PriorityColor[taskData.itemPriority]}] w-fit px-[12px] py-[4px]`}>
                            <p className="text-[17px] text-white capitalize ">{Priority[taskData.itemPriority]}</p>
                        </div>
                    </div>
                </div>
                <div className="py-[18px] border-t-[2px] border-b-[2px] border-[#9F9F9F] my-[24px]">
                    ...
                    <div className=" flex items-center  space-x-[20px] my-[8px] px-[24px]">
                        ...
                        <div className="">
                            <p className=" capitalize  text-[#0D121C] text-[18px]   font-bold">{taskData.createdBy}</p>
                            <p className="text-[#616161] text-[18px]   font-medium capitalize">manager</p>
                        </div>
                    </div>
                </div>
                <div className=" space-y-[8px]">
                    <h3 className="text-[#616161] text-[18px]   font-semibold">Actions</h3>
                    <button type='button'  className="py-[10px] px-[15px] border-[1px] border-transparent rounded-[12px] bg-mainColor text-[17px] text-[#FFFFFF] hover:bg-transparent hover:text-mainColor hover:border-mainColor transition-all duration-300 font-medium" onClick={()=>{CompleteTask(taskData.farmId,taskData.id)}}>
                ...
                <div className="flex justify-end my-[10px]">
                    <button type='button' className="py-[10px] px-[15px]  border-[1px] border-[#616161] rounded-[12px] text-[#333333]  text-[17px]  hover:bg-mainColor hover:text-[#FFFFFF] hover:border-mainColor transition-all duration-300 font-semibold"onClick={(e)=>{if(e.target==e.currentTarget){children.setDisplayTask(null)}}}>
                            <div className="flex justify-center items-center space-x-[11px]">
