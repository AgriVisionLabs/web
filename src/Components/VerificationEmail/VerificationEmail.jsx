...
const VerificationEmail = (children) => {
    ...
    // } 
    useEffect(() => {
        if (count === 0) return; // توقف عند 0

        const timer = setInterval(() => {
        setCount(prev => prev - 1);
        }, 1000); // كل ثانية

        return () => clearInterval(timer); // تنظيف الـ interval عند الخروج
    }, [count]);
    ...
    return (
        <section className="h-[100vh] flex justify-center items-center bg-black bg-opacity-70  font-manrope backdrop-blur-md" onClick={(e)=>{
            if(e.target===e.currentTarget){
                ...
                :<>
                <p className="bg-[#F3F4F6] px-9  py-5 rounded-full text-[#9CA1A4]  font-medium  mt-9">Resend in {count} S</p>
                </> }
