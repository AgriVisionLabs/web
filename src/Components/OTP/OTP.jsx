...
const OTP = ({ emailOfForgotPassword, otpValue, setOtpValue, setResetToken }) => {
    ...
    var validationSchema=object({
        ...
        otpCode:string().required("OTP is required").length(6, "OTP must be 6 digits")
    })
            ...
            if(e.target===e.currentTarget){
                ...
                <div className="absolute top-5 left-5">
                    <button 
                        ...
                        onClick={()=>{setOtp(false); setForgetPassword(true);}}
                        aria-label="Go back"
                    >
                ...
                <div className="flex flex-col items-center justify-center text-center">
                    <div className="flex justify-center items-center mb-6">
                        <div className="w-[70px] h-[70px] bg-[#1e693029] rounded-full flex justify-center items-center">
                            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-mainColor">
                                ...
                                <path d="M22 7L13.03 12.7C12.71 12.89 12.36 13 12 13C11.64 13 11.29 12.89 10.97 12.7L2 7" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                            </svg>
                        </div>
                    </div>
                    <h1 className="text-[25px] text-mainColor capitalize font-medium mb-4">OTP Verification</h1>
                    <div className="flex items-center justify-center mb-4">
                        <p className="w-[400px] text-[16px] text-center text-[#333333]">Enter the verification code we just sent to your email address</p>
                        <div className="group relative ml-2">
                            ...
                            <div className="absolute left-full top-1/2 transform -translate-y-1/2 ml-2 w-64 p-3 bg-gray-800 text-white text-sm rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 pointer-events-none z-50">
                                ...
                                <div className="font-medium text-blue-200 mb-1">OTP Instructions:</div>
                    ...
                    <form action="" className="w-[400px] my-6 flex flex-col items-center" onSubmit={formik.handleSubmit}>
                    <div className='space-x-5'>
                        <Check otpValue={otpValue} setOtpValue={setOtpValue} hasError={otpError}/>
                    </div>
                        <button 
                            type='submit' 
                            ...
                            className={`btn w-full py-5 bg-mainColor text-white hover:bg-transparent hover:border-mainColor border-2 hover:text-mainColor font-medium mt-12 transition-all duration-300 ${
                                loading || otpValue.join("").length !== 6 ? "opacity-50 cursor-not-allowed" : ""
                            }`}
                        >
                        ...
                        <p className="my-3 text-[#333333] text-[15px]">
                            ...
                            <button
                                ...
                                className={`text-mainColor hover:text-green-700 transition-colors duration-300 ${
                                    resendLoading || resendCooldown > 0 ? "opacity-50 cursor-not-allowed" : "cursor-pointer"
                                }`}
                            >
