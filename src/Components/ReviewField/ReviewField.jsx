...
const ReviewField = (children) => {
        ...
        try {
            ...
            if(data){
                if(children.FieldData.IrrigationUnit){createIrrigation(data.id)}
                if(children.FieldData.SensorUnit){createSensor(data.id)}
                children.getFields()
            }
        }catch(error){
            console.log(error)
                ...
                if(e.target===e.currentTarget){
                    ...
                    <form action="" className="w-[85%] my-2 flex-grow  flex flex-col justify-between  text-[18px]">
                        ...
                        <div className="flex mt-4 justify-between items-center">
                            ...
                            <button type='button' className="btn self-end w-[150px] rounded-lg py-4 bg-mainColor text-[16px] text-white hover:bg-transparent hover:border-mainColor border-2 hover:text-mainColor font-medium " onClick={()=>{createField()}}>Create Field</button>
                        </div>
