...
const ReviewField = (children) => {
  ...
  console.log("data", children.FieldData);
  ...
  async function createField() {
    try {
      ...
      if (data) {
        if (children.FieldData.IrrigationUnit) {
          createIrrigation(data.id);
        }
        if (children.FieldData.SensorUnit) {
          createSensor(data.id);
        }
        children.getFields();
      }
      setAddField(null);
  ...
  async function createIrrigation(fieldId) {
    try {
      const options = {
  ...
  async function createSensor(fieldId) {
    try {
      const options = {
        url: `${baseUrl}/farms/${children.farmId}/fields/${fieldId}/SensorUnits`,
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  return (
    ...
    >
      <div className=" w-[600px] h-[660px] border-2 rounded-2xl bg-white flex  flex-col items-center">
        ...
        <div className="flex flex-col justify-center items-center mt-8 mb-5">
          ...
          <div className="w-[100%] rounded-xl flex gap-2  items-center">
            <div className=" flex flex-col items-center ">
              <div className="w-[33px] h-[33px] text-[18px] text-white flex justify-center items-center bg-mainColor rounded-full">
                <p className="">1</p>
              </div>
        ...
        <form className="w-[85%] my-2 flex-grow  flex flex-col justify-between  text-[18px]">
          <div className="flex flex-col  ">
            <p className="text-[19px] font-semibold capitalize text-[#0b0b0bd8] mb-2 ">
              Field Details{" "}
            </p>
            <div className="  grid grid-cols-2 mx-3 ">
              <div className="mb-2 font-medium">
                <p className=" capitalize text-mainColor mb-1 text-[17px]">
                  field name
                </p>
              ...
              <div className="mb-2 font-medium">
                ...
                <p className="text-[15px]">
                  {children.cropType[children.FieldData.CropType].name}
                </p>
              </div>
            </div>
          </div>
